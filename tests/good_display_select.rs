...
fn test_named_display() {
    let wrap1 = Wrap::<i32, DisplayImpl1>::new(123);
    assert_eq!(wrap1.to_string(), "DisplayImpl1");

    let wrap2 = Wrap::<i32, DisplayImplProxy<i32>>::new(456);
...
fn test_named_display_ref() {
    let x = 789;
    let ref1: &Wrap<i32, DisplayImpl1> = Wrap::wrap_ref(&x);
    assert_eq!(ref1.to_string(), "DisplayImpl1");

    let ref2: &Wrap<i32, DisplayImplProxy<i32>> = Wrap::wrap_ref(&x);
